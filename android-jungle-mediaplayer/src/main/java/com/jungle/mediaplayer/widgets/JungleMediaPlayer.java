...
public class JungleMediaPlayer extends MediaPlayerFrame {
    ...
    private Runnable mShowPreviewRunnable = new Runnable() {
        @Override
        public void run() {
            Bitmap bitmap = getFrameAtTime(1000 * 1000);
            showPreview(bitmap);
        }
    };
    ...
    public void setPreviewDataSource(final VideoInfo info) {
        showPreview(null);
        new Thread(new Runnable() {
            @Override
            public void run() {
                mMediaPlayer.setPreviewDataSource(info);
                if (!mMediaPlayer.isLoadingOrPlaying()) {
                    post(mShowPreviewRunnable);
                }
            }
        }).start();
    }
    ...
    public void onPlayBtnClicked() {
        mPlayerListener.onPreviewPlayClicked();
    }
    ...
    public Bitmap getFrameAtTime(long time) {
        return mMediaPlayer.getFrameAtTime(time);
    }
}
