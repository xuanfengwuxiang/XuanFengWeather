package com.xuanfeng.xuanfengweather.widget;
 ...
 */
public class FaceViewNormal extends ImageView {
    ...
    public FaceViewNormal(Context context, AttributeSet attrs) {
        super(context, attrs);
        initPaint();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mFaces != null && mFaces.length >= 1) {
            ...
            for (int i = 0; i < mFaces.length; i++) {
                ...
                if (!mirror) {
                    int cx = face.rect.centerX(); //因为之前对camera做了旋转，所以这里需要转换一下坐   //后置摄像头
                    ...
                    int cx = -face.rect.centerX(); //因为之前对camera做了旋转，所以这里需要转换一下坐  //前置摄像头
                    int cy = face.rect.centerY();
                    RectF rectF = new RectF(getWidth() * cx / 2000f - needWidth / 2, getHeight() * cy / 2000f - needWidth / 2, getWidth() * cx / 2000f + needWidth / 2, getHeight() * cy / 2000f + needWidth / 2);
                    canvas.drawRect(rectF, mLinePaint);
                }
